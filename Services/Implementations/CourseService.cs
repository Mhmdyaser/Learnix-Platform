...
namespace Learnix.Services.Implementations
{
    public class CourseService : GenericService<Course,CourseDto,int> , ICourseService
    {
        ...
        public async Task<(bool Success, string Reason)> EnrollInCourse(int courseId, string studentId)
        {
            try
            {
                ...
                decimal price = course.Price != null ? (decimal)course.Price : 0m;


                if (course.IsFree)
                {
                    var freeEnroll = new Enrollment
                    {
                        CourseId = course.Id,
                        StudentId = student.Id,
                        EnrollementDate = DateTime.Now
                    };
                    _unitOfWork.Enrollements.Add(freeEnroll);

                    _unitOfWork.Complete();

                    await SendEnrollmentEmail(studentId, course, freeEnroll.EnrollementDate);

                    return (true, "Enrollment successful (free course)");
                }


                if (student.Balance < price)
                    return (false, "Insufficient balance");
                //return (false, $"Insufficient balance. Student balance: {student.Balance}, course price: {price}");


                student.Balance -= price;


                decimal instructorShare = price * 0.95m;
                decimal adminShare = price * 0.05m;
                ...
                if (admins.Any())
                {
                    decimal perAdminShare = adminShare / admins.Count;
                    foreach (var admin in admins)
                        admin.Balance += perAdminShare;
                }
                ...
                await SendEnrollmentEmail(studentId, course, enrollment.EnrollementDate);
        ...
        //        decimal price = Convert.ToDecimal(course.Price ?? 0);
        ...
        //    }
        //}
        private async Task SendEnrollmentEmail(string studentId, Course course, DateTime date)
        {
            var studentUser = await _userManager.FindByIdAsync(studentId);
            var instructorUser = await _userManager.FindByIdAsync(course.InstructorID);

            await _emailService.SendEmailAsync(new Email
            {
                ReceiverEmail = studentUser.Email,
                Subject = $"Enrollment Confirmation – {course.Title}",
                Body = $@"
Dear {studentUser.FirstName} {studentUser.LastName},

You have been successfully enrolled in:

**{course.Title}**
Instructor: {instructorUser.FirstName} {instructorUser.LastName}
Enrollment Date: {date:dddd, MMMM dd, yyyy}
...
"
        ...
        public async Task<IEnumerable<CourseListViewModel>> GetPopularCoursesAsync(int count = 6)
        {
        ...
        private async Task<CourseDetailsViewModel> MapToCourseDetailsViewModel(Course course)
        {
            ...
            if (course.Instructor != null)
            {
                ...
                instructorVm = new InstructorViewModel
                {
                    ...
                    AverageRating = Math.Round(instructorStats.AverageRating, 1),
        ...
        private Course MapToCourseEntity(CourseDetailsViewModel courseVm)
        {
            return new Course
            {
                Title = courseVm.Title,
                Description = courseVm.Description,
                LearningOutCome = courseVm.LearningOutCome,
                Duration = courseVm.Duration,
                Requirement = courseVm.Requirement,
                Price = (double?)courseVm.Price,
                IsFree = courseVm.IsFree,
                ImageUrl = courseVm.ImageUrl,

            };
        }

        private void UpdateCourseEntity(Course course, CourseDetailsViewModel courseVm)
        {
            course.Title = courseVm.Title;
            course.Description = courseVm.Description;
            course.LearningOutCome = courseVm.LearningOutCome;
            course.Duration = courseVm.Duration;
            course.Requirement = courseVm.Requirement;
            course.Price = (double?)courseVm.Price;
            course.IsFree = courseVm.IsFree;
            course.ImageUrl = courseVm.ImageUrl;
        }
